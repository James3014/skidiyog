...
if(isset($_SESSION['account'])){
  ...
  $R=$ACCOUNT->get_account($account_info); //_v($R);
  ...
  $query_arry['oidx']     =crypto::dv($_REQUEST['id']);
  if(!is_numeric($query_arry['oidx'])){
    _alert('錯誤保單鏈結，請重新登入!!'.$query_arry['oidx']);
  ...
  if(count($insuranceData)==1){
    ...
    $insuranceResult['pcname']    = $R['name'];
    $insuranceResult['pename']    = '';
    $insuranceResult['birthday']  = '';
    $insuranceResult['sex']       = -1;
    $insuranceResult['email']     = $R['email'];
    $insuranceResult['phone']     = $R['phone'];
    $insuranceResult['address']  = '';
    $insuranceResult['country']  = '';
    $insuranceResult['emergencyName']  = '';
    $insuranceResult['inusrance_num'] =$orderFUNC->order_student_maxnum($query_arry['oidx']);
  ...
  //_v($insuranceResult);
  $POST_CMD = 'insurance_apply';
}elseif($_REQUEST['from']=='INSURM'){ // click from email and no login yet
  _go('account_login.php?from=INSURM&id='.urlencode($_REQUEST['id'])); 
}else{
  _go('insurance_fapply.php?id='.$_REQUEST['id']);
}
/*
$filters = array(
      ...
      $(document).ready(function(){
        $(function(){          
                    ...
                    success: function(resp){
                        //alert("Successfully submitted."+resp)
                            if(resp==101005){ // user profile verify & MAIL CHECK PASS
                                ...
                                //$('#ERRMSG').text('資料填寫不完整');
                                $('#PERRMSG').text('訂單編號讀取錯誤！<?=$query_arry['oidx'];?>');
                                $('#err_msg').modal('open'); 
                                ...
                                $('#err_msg').modal('open');
                            }else if(resp==<?=ERR_NULL_DAY1_DATE?>){
                                $('#PERRMSG').text('第一天上課日期尚未填寫');
                                $('#err_msg').modal('open');                                                                                                                               
                            }else if(resp==100006){
                                //$('#ERRMSG').text('資料填寫不完整');
                                $('#PERRMSG').text('提醒您： 被保人姓名及身分證字號是必填資料唷！');
                                $('#err_msg').modal('open'); 
                            }else{
                                $('#err_msg').modal('open');                                
                            }                         
           ...
           $('#gotit').on('click', function(e){                       
              window.location.replace('class_booking_edit.php?id=<?=urlencode($_REQUEST['id'])?>');
           }); 
        ...
        <div class="container-fuild">
          ...
          <div class="row header-block-login">
            ...
            <div class="col s10 push-s1  m6 push-m3  header-block-content">
                <p class="text-center"><?php echo $R['name']; ?></p>
                <p class="resort-name">保險資訊 <span></span></p>
               ...
               <div class="row">
                 <div class="col s12 l8 col-centered center">
                    ...
                    <form class="col s12 space-top-2" id="accountinfo-form">
                      ...
                      <div class="input-field col s12 m6">
                        <label for="" class="active">被保團員人數: </label>
    
                        <select id="p_amount" name="p_amount">   
                          <option value="1" <?=($insuranceResult['inusrance_num']==1)?'selected':'' ?> >1</option>
                          <option value="2" <?=($insuranceResult['inusrance_num']==2)?'selected':'' ?> >2</option>
                          <option value="3" <?=($insuranceResult['inusrance_num']==3)?'selected':'' ?> >3</option>
                          <option value="4" <?=($insuranceResult['inusrance_num']==4)?'selected':'' ?> >4</option>
                          <option value="5" <?=($insuranceResult['inusrance_num']==5)?'selected':'' ?> >5</option>
                          <option value="6" <?=($insuranceResult['inusrance_num']==6)?'selected':'' ?> >6</option>
                          <option value="7" <?=($insuranceResult['inusrance_num']==7)?'selected':'' ?> >7</option>
                          <option value="8" <?=($insuranceResult['inusrance_num']==8)?'selected':'' ?> >8</option>
                          <option value="9" <?=($insuranceResult['inusrance_num']==9)?'selected':'' ?> >9</option>
                          <option value="10" <?=($insuranceResult['inusrance_num']==10)?'selected':'' ?> >10</option>
                        </select>
                      ...
                      <div class="input-field col s12 m6" style="display:none">
                        ...
                        <input name="p_no" type="text" value="0000" placeholder="（必填）">
                      </div>

                      <div class="input-field col s12 m6">
                        <label for="">同護照之英文名稱</label>
                        <input name="p_en" type="text" value="<?= $insuranceResult['pename']; ?>" placeholder="（必填）">
                      </div>
                      <div class="input-field col s12 m6">
                        <label for="">生日 （yyyy-mm-dd）</label>
                        <input name="birth" type="text" value="<?= $insuranceResult['birthday']; ?>" placeholder="（必填）">
                      </div>
                      <div class="input-field col s12 m6">
                      ...
                      <div class="input-field col s12 m6">
                        ...
                        <input name="tid" type="text" value="<?= $insuranceResult['twid']; ?>" placeholder="（必填）">
                      </div>
                      <!--
                      <div class="input-field col s12 m6">
                        <label for="">密碼</label>
                      ...
                      <div class="input-field col s12 m6">
                        <label for="">手機</label>
                        <input type="text" name="phone" value="<?= $insuranceResult['phone']; ?>" placeholder="（必填）" >
                      </div>
                      <div class="input-field col s12 m6">
                        <label for="">電子信箱</label>
                        <input type="text" name="mail" value="<?= $insuranceResult['email']; ?>" placeholder="（必填）" >
                      </div>  
                      <div class="input-field col s12 m6">
                        <label for="">地址</label>
                     ...
                     <input type="hidden" name="oidx" value="<?php echo $_REQUEST['id']; ?>" >
                      ...
                      在我們投保後，您自行加保的保單會被富邦系統擋下，本次行程請改用「富邦產險以外」的旅平險。 <br>
                        ...
                        <button id="modifybt"  class="btn waves-effect waves-light btn-primary space-top-2 modal-trigger" type="submit" name="action">儲存 <i class="material-icons">chevron_right</i></button>
