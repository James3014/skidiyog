...
$ORDER_OBJ = new ORDER();
/*
if(!empty($_REQUEST['q']) && $_REQUEST['q']=='yes'){
    //echo 'xx';
    //var_dump($in);
    $sql_where = "WHERE MONTH(createDateTime) = ".$in['month']." AND YEAR(createDateTime) = ".$in['year']." AND status='".$in['status']."' ";
    $order_query_result = $ORDER_OBJ->sql_query($sql_where);   
}
*/
    /*
    $sql_where = "WHERE MONTH(createDateTime) = ".$in['month']." AND YEAR(createDateTime) = ".$in['year']." AND status='".$in['status']."' ";
    if($in['status']=='all'){
        $sql_where = "WHERE MONTH(createDateTime) = ".$in['month']." AND YEAR(createDateTime) = ".$in['year'] ;
    }
    */
    $sql_where = '';
    if($in['status']!='all'){
        $sql_where = "WHERE status='".$in['status']."' ";
    }
          ...
          <div class="col s12 m6 offset-m1 pull-m3 " >
            <div class="row space-top-1 row-margin-b0">              
              <div class="col s11 col-centered" id="private">
                    ...
                    -->   
                    <?php echo '共計'.count($order_query_result).'筆訂單';?>              
              ...
              <div class="col s11 col-centered" id="private2">
                    ...
                    <table class="order">                   
                        <thead>
                         <tr>
                           <td colspan="3">
                            <b>訂單編號:#<?=$order['oidx'].$order['status']?></b><br>                   
                           </td>
                           <td><b>訂單日期: <?=$order['schedule'][0]['date'] ?></b></td>
                         </tr>                         
                          <tr>
                              <th width="30%"><p class="left">日期 時間/堂次</p></th>
                              <th width="40%"><p class="left">雪場 教練/種類</p></th>
                        ...
                        <?php foreach ($order['schedule'] as $n => $o) {//_v($o); ?>
                         ...
                         <tr>
                           <td rowspan="5" colspan="2">
                            <b>學生姓名：</b><?=$student['name']?><br><br>
                            <b>EMAIL： </b><?=$student['email']?>, <b>手機：</b> +<?=$student['country']?> <?=$student['phone']?><br><br>
                            <b>FB:</b> <?=$student['fbid']?>, <b>Line ID：</b> <?=$student['line']?>, <b>WeChat：</b> <?=$student['wechat']?>  <br>  <br>                                                   
                            <b>學生備註：</b><?=$order['requirement']?><br><br>
                            <b>管理備註：</b><?=$order['note']?><br><br>
