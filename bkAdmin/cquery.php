<?php
    ...
    if(empty($in['class_date_s'])) $in['class_date_s']='2019-10-01';
    if(empty($in['class_date_e'])) $in['class_date_e']='2020-04-30';
    ...
    if(!empty($_REQUEST['year']) ){
      $instructors_levelfee = 0;
      $query_arry['type']     ='QUERY_COST';
      ...
      $query_arry['q_instructor']=$_REQUEST['istatus']  ; // workaround for ths status filed in form
      $where_arry['name'] =  $query_arry['q_instructor'];      
      $where_arry['type'] =  'instructor';
      $instructors_info = $ko->getInstructorInfo($where_arry);
      if($query_arry['q_instructor']!='all')
      $instructors_levelFee = $instructors_info[$where_arry['name']]['levelFee'];

      //echo '>>'.$query_arry['q_instructor'];
      //var_dump($instructors_info);     
...
<!--
      <div class="input-field col s3" >
        ...
        <label class="active"><span></span>訂單日期</label>
      </div>
      <div class="input-field col s3" >
    ...
    -->
      <div class="input-field col s3" >
        ...
        <label class="active"><span></span>上課日期</label>
      </div>      
      <div class="input-field col s3" >
...
-->
          <option <?=(isset($_REQUEST['istatus']) && $_REQUEST['istatus']=='all')?'selected':''?> value="all">🌀 不限</option>
                      ...
                      ?>
                        <option <?=(isset($_REQUEST['istatus']) && $_REQUEST['istatus']==$name)?'selected':''?> value="<?=$name?>" data-icon="https://diy.ski/photos/<?=$name?>/<?=$name?>.jpg"
                          ><?=$instructor['cname']?></option>
                      <?php 
                          }
                        } // end of foreach 
                      ?> 
        ...
        <label class="active"><span></span>教練</label>
      </div>
    ...
    <thead>
      <tr bgcolor="#b8b6b6">
<?php
      if(!empty($query_arry['q_instructor']) && $query_arry['q_instructor']=='all'){
        echo '<th width="3%"><p class="left">教練</th>';
      }
?>        
<!--
        <th width="3%"><p class="left">筆數</th>
        <th width="5%"><p class="left">時間</th>
        <th width="2%"><p class="left">堂</th>
        <th width="7%"><p class="left">地點代號</th>
        <th width="3%"><p class="left">學生數</th>
        <th width="3%"><p class="left">保險送核數</th>
        <th width="5%"><p class="left">種類</th>
        <th width="5%"><p class="left">訂單編號</th>        
        <th width="5%"><p class="left">該堂學費</th>  
        <th width="5%"><p class="left">訂單總學費</th>
        <th width="5%"><p class="left">總收訂金</th>  
        <th width="5%"><p class="left">實收訂金</th>  
        <th width="5%"><p class="left">當時匯率</th>
        <th width="5%"><p class="left">總收尾款</th>
        <th width="5%"><p class="left">加價</th>
        <th width="5%"><p class="left">總學費</th>
        <th width="5%"><p class="left">費用累加</th>
        <th width="5%"><p class="left">手續費</th>
        <th width="5%"><p class="left">餘額(NT)</th>
        <th width="5%"><p class="left">訂單備註</th>
        <th width="5%"><p class="left">系統備註</th>
-->
        <th width="3%"><p class="left">筆數</th>
        <th width="5%"><p class="left">時間</th>
        <th width="2%"><p class="left">堂</th>
...
<?php
    ...
    if(!empty($insuranceResult)){ 
      ...
      foreach ($insuranceResult as $key => $value) {
        $order_use_check[$value['oidx']]=0;
        $insurance_max[$value['oidx']]=0;
      }
      // set min & max insurance count
      foreach ($insuranceResult as $key => $value) {        
        if($value['studentNum'] > $insurance_max[$value['oidx']]) $insurance_max[$value['oidx']] = $value['studentNum'];
      }      
    }
    ...
    foreach ($insuranceResult as $key => $value) {
      ...
      $real_income_prepaid =0;


      
      if($order_use_check[$value['oidx']]!=99){ // 該訂單第一次出現
        $insurance_count = $insurance_max[$value['oidx']];
        
        if($insuranceFUNC->istatus_count($value['oidx'],'pass')>0){
          $insurance_count = $insuranceFUNC->istatus_count($value['oidx'],'pass'); // 實際通過保險人數
        } 
               
        if($query_arry['q_instructor']=='all'){
          $where_arry['name'] =  $value['instructor'];      
          $where_arry['type'] =  'instructor';
          $instructors_info = $ko->getInstructorInfo($where_arry);          
          $instructors_levelFee = $instructors_info[$value['instructor']]['levelFee'];
        }

        $real_income_prepaid = $value['prepaid']-($insurance_count*1000); // 實收訂金
        $total_tail_payment= $value['price']-$value['discount']-$value['prepaid']; //總收尾款 
        $total_fee = $total_tail_payment + ($real_income_prepaid) - $instructors_levelFee; // 總學費 = （總收尾䤭）＋實收訂金 - 加價     
        $balance = ($real_income_prepaid  -($total_fee * $processing_fee )) * $value['exchangeRate'];        
        $order_use_check[$value['oidx']] = 99;
        $order_show_full ='y';
        $total_ammount = $total_ammount + $total_fee;
        $total_balance = $total_balance + $balance ;
        $total_pfee = $total_pfee + ($total_fee * $value['exchangeRate'] * $processing_fee );
        //$insurance_count = $value['studentNum'];
      }else{ // 該訂單已出現過
        $order_show_full ='n';
        $bgcolor='#dedede';
        //if($value['studentNum']>$insurance_count)  $insurance_count = $value['studentNum']; // 大的取代
      }
...
<!--
    ...
    <td><?=$value['oidx'] ?></td>
    <td><?=$value['fee'] ?></td>
    <td><?= ($order_show_full=='n')?'':$value['price'] ?></td>
    <td><?= ($order_show_full=='n')?'':$value['prepaid']?></td>
    <td><?= ($order_show_full=='n')?'':$real_income_prepaid?></td>
    <td><?= ($order_show_full=='n')?'':$value['exchangeRate']?></td>
    <td><?= ($order_show_full=='n')?'':$total_tail_payment ?></td> 
    <td><?= ($order_show_full=='n')?'':$instructors_levelFee?> </td>
    <td><?= ($order_show_full=='n')?'':$total_fee?></td> 
    <td><?= ($order_show_full=='n')?'':$total_ammount?></td>
    <td><?= ($order_show_full=='n')?'':($processing_fee *100).'%' ?></td>
    <td><?= ($order_show_full=='n')?'':$balance?></td>
    <td><?= ($order_show_full=='n')?'':$value['memo']?></td>
    <td><?= ($order_show_full=='n')?'':$value['note']?></td>    
...
-->
    ...
    <td><?= ($order_show_full=='n')?'':$value['prepaid']?></td>    
    <td><?= ($order_show_full=='n')?'':$value['exchangeRate']?></td>
    <td><?= ($order_show_full=='n')?'':$total_tail_payment ?></td> <!-- 總收尾款 -->
    <td><?= ($order_show_full=='n')?'':$instructors_levelFee?> </td>
    <td><?= ($order_show_full=='n')?'':$value['memo']?></td>
    <td><?= ($order_show_full=='n')?'':$value['note']?></td>  
    <td><?= ($order_show_full=='n')?'':($processing_fee *100).'%' ?></td>
...
<!--
    <td></td>    
    <td></td>     
    <td><cR><strong><?=$total_ammount?></strong></cR></td>
    
    <td><cR><strong><?=$total_balance?></strong></cR></td>
-->    
      ...
      $(document).ready(function(){
          ...
          var act_date2 = new Date(today - (86400000 * 365 * 2)); // 可查前半年
