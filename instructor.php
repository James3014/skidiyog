<?php
    ...
    if(empty($name)){//from old link : https://diy.ski/instructor.php?name=xxx
      require('includes/sdk.php');
      $DB = new db();
      $name = $_REQUEST['name'];
      //copy form routing.php
      $sql = "SELECT * FROM `instructors` WHERE `name`='{$name}'"; //_d($sql);
    ...
    if(in_array($name, ['phil','wawa','eden','tommy','sophia'])){
      Header('Location: https://diy.ski/instructorList.php');
      exit();
    }
    ...
    $instructor_basic_info = $INSTRUCTORS->get_instructor_profile_by_Name($name);
    //var_dump($instructor_basic_info);
    $instructor_basic_info['cname'] = (stripos($instructor_basic_info['cname'], '放肆客')!==false) ? '放肆客' : $instructor_basic_info['cname'];

    $expertise = '';
    if($instructor_basic_info['expertise']=='sb')   $expertise = "Snowboard";
    if($instructor_basic_info['expertise']=='ski')  $expertise = "Ski";
    if($instructor_basic_info['expertise']=='both') $expertise = "Snowboard & Ski";
    $expertise = ($instructor_basic_info['name']==='firsttrack') ? 'Snowboard & Ski' : $expertise;
?>
      ...
      $(document).ready(function(){
        $(function(){          
               $('#ordernow').on('click', function(e){         
                    window.location.replace('../schedule.php?f=i') 
               }); 
        });
      });  
    </script>
      ...
      <div class="container-fuild">
        ...
        <div class="row header-block-resort">
            ...
            <div class="col s10 push-s1  m6 push-m3  header-block-content">              
              <p class="resort-name"><?=$instructor_basic_info['cname']?><br><small><?=$expertise?></small></p> 
              <!--<p class="resort-name"> <small><?=$expertise?></small></p> -->
      ...
      <div class="container resort-info">
        <div class="row">
            <div class="col l3 hide-on-med-and-down leftnav">
              <p class="resort-name"><?=$instructor_basic_info['cname']?></p>
...
?>
            ...
            <div class="col s12 l9 right resort-content">                 
              ...
              if(!empty($section) && $section != 'all'){
                ...
                echo '<p>'.nl2br($section_content[$section]).'</p><hr>';
